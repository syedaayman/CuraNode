...
// POLISHED DEDICATED ARRIVAL SUCCESS UI FOR WHEN AMBULANCE HAS ARRIVED
function ArrivalSuccessUI({
  ...
  return (
    <main className="w-full max-w-[1140px] mx-auto px-4 sm:px-6 py-4 flex-1 flex flex-col justify-center box-border bg-[#F8FFFD] animate-in fade-in duration-300">
      
      {/* UNIFIED SINGLE HERO / EMERGENCY REACHED CARD */}
      <section className="relative rounded-[24px] border border-[#D5EDE8] bg-gradient-to-r from-[#F0FAF8] via-[#F4FAF9] to-[#EBF7F5] p-6 sm:p-8 md:p-10 shadow-sm shrink-0 overflow-hidden">
        ...
        <div className="absolute top-1/2 left-1/3 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-[#19B5B1]/10 rounded-full blur-3xl pointer-events-none -z-0" />

        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-center text-left">
          
          {/* LEFT SIDE: Status, Title, ECG & Reassurance (7 Cols) */}
          <div className="lg:col-span-7 space-y-4">
            
            {/* Arrived Badge & Pill */}
            <div className="flex items-center space-x-3">
              <div className="relative w-11 h-11 rounded-full bg-[#087F7B] text-white flex items-center justify-center text-xl font-bold shadow-[0_4px_14px_rgba(8,127,123,0.25)] ring-4 ring-[#EAF8F5]">
                ✓
              </div>
              
              <span className="bg-[#EAF8F5] text-[#087F7B] border border-[#BDE9E4] px-3.5 py-1 rounded-full font-ui font-bold text-xs uppercase tracking-wider inline-flex items-center gap-1.5 shadow-2xs">
            ...
            <div className="space-y-2 pt-1">
              <h1 className="font-display text-3xl sm:text-4xl lg:text-[38px] font-bold text-[#112F35] tracking-tight leading-[1.15]">
                Our medical team <br className="hidden sm:inline" />
                <span className="text-[#087F7B]">is with you now.</span>
              </h1>
              
              {/* ECG Pulse Heartbeat Line */}
              <div className="py-1">
                <svg className="w-32 h-5 text-[#19B5B1] opacity-80" viewBox="0 0 120 20" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M0 10 H40 L46 2 L52 18 L58 5 L64 13 L70 10 H120" />
                </svg>
              </div>

              {/* Status & Reassurance Messages */}
              <div className="space-y-1.5 pt-1">
                <p className="font-body text-sm sm:text-base text-[#334155] font-semibold flex items-center space-x-2">
                  <span className="w-2 h-2 rounded-full bg-[#087F7B] shrink-0"></span>
                  <span>Responders are on-site providing medical assistance.</span>
                </p>
                <p className="font-body text-xs sm:text-sm text-[#087F7B] font-medium flex items-center space-x-1.5">
                  <span>Your safety is our priority. Focus on getting the care you need.</span>
                  <span>💚</span>
                </p>
              </div>
            </div>

            {/* Hospital Base Details */}
            {hospital && (
              <div className="pt-2 border-t border-[#BDE9E4]/60 flex items-center space-x-2 text-xs font-semibold text-[#475569]">
                <span className="text-rose-500">🏥</span>
                <span>Dispatched from <strong className="text-[#112F35]">{hospital.name}</strong></span>
              </div>
            )}

          </div>

          {/* RIGHT SIDE: Naturally Blended Ambulance Illustration (5 Cols) */}
          <div className="lg:col-span-5 flex justify-center items-center relative">
            <div className="w-full max-w-[460px] relative flex items-center justify-center">
              {/* Soft background mint glow extending behind ambulance */}
              <div className="absolute inset-0 bg-gradient-to-tr from-[#19B5B1]/20 via-[#BDE9E4]/30 to-transparent rounded-3xl blur-2xl -z-0" />
              
              <img 
                src="/images/curanode_ambulance.png" 
                alt="CuraNode Emergency Ambulance" 
                className="relative z-10 w-full h-auto object-contain rounded-2xl filter drop-shadow-[0_8px_20px_rgba(8,127,123,0.15)] transition-transform duration-300 hover:scale-[1.01] mix-blend-multiply"
              />
            </div>
          </div>
      ...
      {/* Footer */}
      <footer className="w-full pt-3 pb-1 text-center shrink-0">
        <div className="font-body text-xs text-[#64748B] font-medium flex items-center justify-center space-x-1.5">
          <span>🛡</span>
          <span>CuraNode Emergency Telemetry Network © {new Date().getFullYear()} — All Rights Reserved.</span>
        </div>
      </footer>
...
export function TrackingContent({
  ...
  const [simulatedEta, setSimulatedEta] = useState<string>("2 mins");
  ...
  // Ambulance Movement Simulation along Real OSRM Road Polyline (Completes in ~120 Seconds)
  useEffect(() => {
    if (currentState < 5 || !routeCoords || routeCoords.length < 2) return;

    const DURATION_SECONDS = 120; // Exactly ~120 seconds simulation duration for demonstration
    ...
    const interval = setInterval(() => {
      elapsedSeconds += 1;
      const progress = Math.min(1, elapsedSeconds / DURATION_SECONDS);
      const currentIdx = Math.min(totalPoints - 1, Math.floor(progress * (totalPoints - 1)));

      if (progress >= 1) {
        setSimulatedAmbulancePos(routeCoords[totalPoints - 1]);
        setIsAmbulanceArrived(true);
        setJourneyStep(3); // Arrived at Location (final 3rd step)
      ...
      const remainingSeconds = DURATION_SECONDS - elapsedSeconds;
      const remainingMinutes = Math.max(1, Math.ceil(remainingSeconds / 60));
      const formattedEta = `${remainingMinutes} min${remainingMinutes > 1 ? "s" : ""}`;
  ...
  return (
    <div className="w-full min-h-screen lg:h-[100dvh] lg:max-h-[100dvh] bg-[#F8FBFA] text-[#16232D] font-sans flex flex-col overflow-x-hidden lg:overflow-hidden relative select-none">
      ...
      <header className="w-full h-[64px] min-h-[64px] max-h-[64px] bg-white border-b border-[#E3EEEC] px-4 sm:px-6 flex items-center justify-between z-30 shadow-2xs shrink-0">
        {/* Left Logo & Main Brand */}
        <div className="flex items-center space-x-3.5">
          <Logo size="md" />
        </div>

        {/* Right Active Emergency Badge */}
        <div className="flex items-center space-x-2 bg-rose-50 border border-rose-200 px-3.5 py-1.5 rounded-xl shadow-2xs">
          <span className="relative flex h-2.5 w-2.5">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-rose-500 opacity-75"></span>
            <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-rose-600"></span>
          </span>
          <span className="font-ui text-xs sm:text-[12.5px] font-extrabold text-rose-700 uppercase tracking-wide shrink-0">
            EMERGENCY ACTIVE
          </span>
        </div>
      </header>
          ...
          <div className="bg-white rounded-2xl p-5 sm:p-7 border border-[#E2E8F0] shadow-sm max-w-[540px] w-full space-y-4 text-center relative overflow-hidden shrink-0">
            ...
            <div className="space-y-2">
              <div className="flex flex-col items-center">
                <span className="bg-[#FFF8EB] text-[#D97706] border border-[#FDE68A] px-4 py-1 rounded-full font-bold text-xs uppercase tracking-wider inline-flex items-center gap-1.5 shadow-2xs">
                  <span className="relative flex h-2.5 w-2.5">
              ...
              <div className="space-y-1">
                <h1 className="text-2xl sm:text-3xl font-bold text-[#112F35] tracking-tight">
                ...
                {canonicalIncidentId && (
                  <div className="text-xs font-mono text-[#087F7B] font-semibold tracking-wide pt-0.5">
            ...
            <div className="bg-white rounded-xl p-4 sm:p-4.5 border border-[#E2E8F0] shadow-2xs text-left space-y-2">
              <div className="flex items-center justify-between border-b border-[#F1F5F9] pb-2">
                <span className="text-xs font-bold text-[#64748B] uppercase tracking-wider">
                  REQUEST SENT TO
                </span>
                <span className="text-xs font-bold text-[#059669] bg-[#ECFDF5] px-2.5 py-0.5 rounded-md border border-[#A7F3D0]">
                  ✓ Request Transmitted
                </span>
              </div>

              <div className="space-y-1">
                <div className="text-lg sm:text-xl font-bold text-[#112F35]">
                  {hospital?.name || "BGS Global Hospital"}
                </div>
                <div className="text-sm text-[#087F7B] font-medium flex items-center space-x-1.5">
                  <span className="w-2.5 h-2.5 rounded-full bg-[#22B573] animate-pulse shrink-0"></span>
            ...
            <div className="bg-white rounded-xl p-3.5 border border-[#E2E8F0] shadow-2xs">
              <div className="relative w-full flex items-center justify-between text-center">
                ...
                <div className="relative z-10 flex flex-col items-center w-1/3">
                  <div className="w-5 h-5 rounded-full bg-[#19B5B1] text-white font-bold text-xs flex items-center justify-center border-2 border-white shadow-2xs">
                    ✓
                  </div>
                  <span className="text-[11.5px] font-bold text-[#087F7B] mt-1.5">Request Sent</span>
                </div>

                {/* Step 2: Hospital Review (Active/Waiting) */}
                <div className="relative z-10 flex flex-col items-center w-1/3">
                  <div className="w-5 h-5 rounded-full bg-[#FFF8EB] border-2 border-[#D97706] text-[#D97706] font-bold text-[10px] flex items-center justify-center shadow-2xs animate-pulse ring-2 ring-[#F59E0B]/20">
                    ●
                  </div>
                  <span className="text-[11.5px] font-extrabold text-[#D97706] mt-1.5">Hospital Review</span>
                </div>

                {/* Step 3: Accepted (Pending) */}
                <div className="relative z-10 flex flex-col items-center w-1/3">
                  <div className="w-5 h-5 rounded-full bg-white border-2 border-[#CBD5E1] text-[#94A3B8] font-semibold text-[10px] flex items-center justify-center">
                    ○
                  </div>
                  <span className="text-[11.5px] font-semibold text-[#94A3B8] mt-1.5">Accepted</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-[#F4FAF9] rounded-xl p-4 border border-[#BDE9E4] text-center space-y-1 shadow-2xs">
              <div className="text-xs font-extrabold text-[#087F7B] uppercase tracking-wider">
                AUTOMATED ESCALATION
              </div>
              
              <div className="text-4xl sm:text-5xl font-extrabold text-[#087F7B] font-mono tracking-tight my-1 leading-none">
              ...
              <div className="text-sm font-bold text-[#112F35]">
            ...
            <div className="pt-0.5">
              <div className="text-sm font-semibold text-[#087F7B] inline-flex items-center justify-center space-x-1.5 bg-[#EAF8F5] px-4 py-1.5 rounded-full border border-[#BDE9E4]/60">
                <span>💚</span>
            ...
            <div className="pt-1">
              <button
                ...
                className="w-full h-[42px] bg-white hover:bg-rose-50/80 border border-rose-200 hover:border-rose-300 text-rose-600 rounded-xl font-bold text-sm transition-colors flex items-center justify-center space-x-1.5 cursor-pointer active:scale-[0.99]"
              >
                <span>✕</span>
        ...
        <main className="w-full max-w-[1400px] mx-auto px-3 sm:px-5 pt-2.5 pb-2 flex-1 min-h-0 flex flex-col justify-between overflow-y-auto lg:overflow-hidden box-border">
          ...
          <section className="grid grid-cols-1 lg:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)] gap-3 flex-1 min-h-0 overflow-visible lg:overflow-hidden items-stretch">
            ...
            <div className="flex flex-col gap-2 h-full min-h-0 overflow-hidden text-left">
              ...
              <div className="rounded-[14px] border border-[#E3EEEC] bg-white p-2.5 shadow-[0_4px_20px_rgba(20,70,65,0.05)] shrink-0">
                <div className="mb-1 flex items-center justify-between">
                  <span className="text-[10.5px] font-black tracking-wider text-[#087F7B] uppercase">
                    ACTIVE EMERGENCY • <span className="font-mono text-[#19B5B1]">ID: {canonicalIncidentId}</span>
                  </span>

                  <span className="rounded-full bg-[#EAF8F0] px-2.5 py-0.5 text-[10.5px] font-extrabold text-[#22B573] uppercase tracking-wider border border-[#22B573]/20">
                    ACCEPTED
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h1 className="text-lg font-black text-[#16232D] leading-tight tracking-tight">
                      Emergency Accepted
                    </h1>

                    <p className="text-xs text-[#7A8790] font-semibold mt-0.5">
              ...
              <div className="rounded-[14px] border border-[#E3EEEC] bg-white p-2.5 shadow-[0_4px_20px_rgba(20,70,65,0.05)] space-y-1.5 shrink-0">
                <div className="flex items-center justify-between border-b border-[#E3EEEC] pb-1">
                  <div className="flex items-center space-x-1.5">
                    ...
                    <h3 className="text-xs font-black text-[#16232D] uppercase tracking-wider">AMBULANCE ON THE WAY</h3>
                  </div>
                  <span className="rounded-full bg-[#EAF8F0] px-2.5 py-0.5 text-[10px] font-extrabold text-[#087F7B] uppercase tracking-wider border border-[#087F7B]/20">
                    EN ROUTE
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-base font-extrabold text-[#16232D]">
                ...
                <div className="space-y-1 text-xs pt-1 border-t border-[#E3EEEC]">
                  <div className="flex items-start space-x-1.5">
                    <span className="w-3.5 h-3.5 rounded-full bg-sky-100 text-sky-600 flex items-center justify-center font-bold text-[8px] shrink-0 mt-0.5">📍</span>
                    <div>
                      <div className="text-[9px] font-bold text-[#7A8790] uppercase tracking-wider">DESTINATION</div>
                  ...
                  <div className="flex items-start space-x-1.5">
                    <span className="w-3.5 h-3.5 rounded-full bg-rose-100 text-rose-600 flex items-center justify-center font-bold text-[8px] shrink-0 mt-0.5">🏥</span>
                    <div>
                      <div className="text-[9px] font-bold text-[#7A8790] uppercase tracking-wider">BASE</div>
                ...
                <div className="grid grid-cols-3 gap-1.5 pt-0.5">
                  <div className="rounded-lg bg-[#F7FBFA] border border-[#E3EEEC] p-1.5 text-center shadow-2xs">
                    <div className="text-[9.5px] font-bold text-[#7A8790] uppercase tracking-wider">ETA</div>
                    <div className="text-base font-extrabold text-[#087F7B] mt-0.5">{simulatedEta}</div>
                  </div>
                  <div className="rounded-lg bg-[#F7FBFA] border border-[#E3EEEC] p-1.5 text-center shadow-2xs">
                    <div className="text-[9.5px] font-bold text-[#7A8790] uppercase tracking-wider">DISTANCE</div>
                    <div className="text-base font-extrabold text-[#16232D] mt-0.5">{simulatedDistance}</div>
                  </div>
                  <div className="rounded-lg bg-[#F7FBFA] border border-[#E3EEEC] p-1.5 text-center shadow-2xs">
                    <div className="text-[9.5px] font-bold text-[#7A8790] uppercase tracking-wider">SPEED</div>
                    <div className="text-base font-extrabold text-[#16232D] mt-0.5">48 km/h</div>
                  </div>
                </div>
              </div>

              {/* CARD 3 — JOURNEY PROGRESS (3 STEPS ONLY) */}
              <div 
              ...
              >
                ...
                >
                  ...
                  >
                    <div 
                      ...
                      style={{ 
                        height: "100%", 
                        background: "#14b8a6", 
                        width: journeyStep === 1 ? '0%' : journeyStep === 2 ? '50%' : '100%' 
                      }} 
                    />
                  </div>
                  ...
                  <div 
                    ...
                    style={{ position: "relative", width: "33.333%", minWidth: 0, display: "flex", flexDirection: "column", alignItems: "center", overflow: "visible", zIndex: 2 }}
                  >
                    <div 
                      className="journey-circle" 
                      style={{ position: "relative", zIndex: 3, width: "22px", height: "22px", minWidth: "22px", minHeight: "22px", boxSizing: "border-box", display: "flex", alignItems: "center", justifyContent: "center", borderRadius: "50%", background: journeyStep >= 1 ? "#14b8a6" : "#ffffff", border: journeyStep >= 1 ? "2px solid #14b8a6" : "2px solid #cbd5e1", color: journeyStep >= 1 ? "#ffffff" : "#64748b", fontSize: "11px", fontWeight: 700, lineHeight: 1 }}
                    >
                      ✓
                    </div>
                    <div 
                      className="journey-label" 
                      style={{ display: "block", marginTop: "6px", width: "100%", textAlign: "center", color: journeyStep >= 1 ? "#0f766e" : "#64748b", fontSize: "10.5px", lineHeight: "13px", fontWeight: journeyStep >= 1 ? 700 : 600, whiteSpace: "nowrap" }}
                    >
                      Request Accepted
                    </div>
                  </div>
                  ...
                  >
                    <div 
                      className="journey-circle" 
                      style={{ position: "relative", zIndex: 3, width: "22px", height: "22px", minWidth: "22px", minHeight: "22px", boxSizing: "border-box", display: "flex", alignItems: "center", justifyContent: "center", borderRadius: "50%", background: journeyStep >= 2 ? "#14b8a6" : "#ffffff", border: journeyStep >= 2 ? "2px solid #14b8a6" : "2px solid #cbd5e1", color: journeyStep >= 2 ? "#ffffff" : "#64748b", fontSize: "11px", fontWeight: 700, lineHeight: 1, boxShadow: journeyStep === 2 ? "0 0 0 4px rgba(20, 184, 166, 0.12)" : "none" }}
                    >
                      {journeyStep >= 2 ? "●" : "2"}
                    </div>
                    <div 
                      className="journey-label" 
                      style={{ display: "block", marginTop: "6px", width: "100%", textAlign: "center", color: journeyStep >= 2 ? "#0f766e" : "#64748b", fontSize: "10.5px", lineHeight: "13px", fontWeight: journeyStep >= 2 ? 700 : 600, whiteSpace: "nowrap" }}
                    >
                      En Route
                    </div>
                  </div>
                  ...
                  >
                    <div 
                      className="journey-circle" 
                      style={{ position: "relative", zIndex: 3, width: "22px", height: "22px", minWidth: "22px", minHeight: "22px", boxSizing: "border-box", display: "flex", alignItems: "center", justifyContent: "center", borderRadius: "50%", background: journeyStep >= 3 ? "#14b8a6" : "#ffffff", border: journeyStep >= 3 ? "2px solid #14b8a6" : "2px solid #cbd5e1", color: journeyStep >= 3 ? "#ffffff" : "#64748b", fontSize: "11px", fontWeight: 700, lineHeight: 1, boxShadow: journeyStep === 3 ? "0 0 0 4px rgba(20, 184, 166, 0.12)" : "none" }}
                    >
                      {journeyStep >= 3 ? "✓" : "3"}
                    </div>
                    <div 
                      className="journey-label" 
                      style={{ display: "block", marginTop: "6px", width: "100%", textAlign: "center", color: journeyStep >= 3 ? "#0f766e" : "#64748b", fontSize: "10.5px", lineHeight: "13px", fontWeight: journeyStep >= 3 ? 700 : 600, whiteSpace: "nowrap" }}
                    >
                      Arrived at Location
                    </div>
                  </div>
              ...
              {/* CARD 4 — LIVE UPDATES */}
              <div className="rounded-[14px] border border-[#E3EEEC] bg-white p-2.5 shadow-[0_4px_20px_rgba(20,70,65,0.05)] flex-1 min-h-[70px] flex flex-col overflow-hidden">
                <h3 className="text-xs font-black text-[#16232D] uppercase tracking-wider shrink-0 mb-1.5">LIVE UPDATES</h3>
                <div className="space-y-1.5 flex-1 min-h-0 overflow-y-auto custom-scrollbar pr-1">
                  {logs.map((log, idx) => (
                    <div key={idx} className="flex items-start space-x-2 text-xs">
                      <span className="text-[10px] font-mono font-bold text-[#64748B] whitespace-nowrap pt-0.5">{log.timestamp}</span>
                      <span className="w-1.5 h-1.5 rounded-full bg-[#19B5B1] shrink-0 mt-1.5"></span>
                      <span className="text-[#16232D] font-medium leading-tight text-[11.5px]">{log.message}</span>
                    </div>
                  ))}
                </div>
              </div>

              {/* ACTION BUTTON (CANCEL REQUEST ONLY) */}
              <div className="shrink-0 pt-0.5">
                <button 
                  onClick={handleCancelRequest}
                  className="w-full h-[36px] bg-white hover:bg-[#FFF1F1] border border-[#F3D3D3] text-[#E76F6F] rounded-[8px] font-extrabold text-xs shadow-2xs transition-all text-center flex items-center justify-center cursor-pointer"
                >
                  ✕ Cancel Request
                </button>
              </div>
            ...
            <div className="w-full h-[320px] sm:h-[420px] lg:h-full min-h-[280px] bg-white rounded-[16px] border border-[#DCEAE7] overflow-hidden relative shadow-2xs flex flex-col p-1 mt-2 lg:mt-0">
              <div className="absolute top-2.5 left-2.5 z-20 bg-white/95 backdrop-blur-xs border border-[#E3EEEC] shadow-2xs rounded-full px-3 py-1 text-[10.5px] font-extrabold text-[#16232D] flex items-center space-x-1.5">
                <span className="w-2 h-2 rounded-full bg-[#22B573] animate-ping"></span>
              ...
              <div className="absolute bottom-3 right-3 bg-white/95 backdrop-blur-md border border-[#E3EEEC] shadow-md rounded-xl p-3 z-20 text-center min-w-[145px]">
                <div className="text-[9.5px] font-extrabold text-[#7A8790] uppercase tracking-wider">ESTIMATED ARRIVAL</div>
                <div className="text-xl font-black text-[#19B5B1] my-0.5">{simulatedEta}</div>
                <div className="text-xs font-extrabold text-[#16232D]">{simulatedDistance} away</div>
                <div className="text-[9px] text-[#7A8790] mt-0.5 font-medium italic">Help is on the way.</div>
              </div>
            </div>
          </section>
          ...
          <div className="w-full py-1 text-center shrink-0">
            <div className="text-sm font-bold text-[#22B573] flex items-center justify-center space-x-1.5">
              <span>🛡</span>
          ...
          {/* Compact Page Footer */}
          <footer className="w-full border-t border-[#E3EEEC] bg-white py-2 px-6 text-center text-xs text-[#6B7780] font-medium shrink-0">
