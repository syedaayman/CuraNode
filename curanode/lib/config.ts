...
export const API_BASE_URL = (process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8001").replace(/\/+$/, "");
...
export const getWsUrl = (): string => {
  if (typeof window !== "undefined") {
    const rawUrl = API_BASE_URL.replace(/\/+$/, "");
    const protocol = rawUrl.startsWith("https:") ? "wss:" : "ws:";
